jQuery(function () {
  jQuery(".widget").localizationTool({
    ...
    strings: {
      ...
      "Dodatkowo z zaznaczonej lokacji można po zwykłych wyjściach się poruszać za pomocą klawiatury numerycznej": {
        en_GB:
          "Additionally from selected location you can move around passing through regulard exits by using num keyboard",
      },
      Autor: {
        en_GB: "Author",
      },
  ...
  getActiveLanguages().forEach((langCode) => {
    let button = jQuery(
      '<a class="dropdown-item" href="#"><div class="flag ' +
        getFlagClassFromLangCode(langCode) +
        '"></div></a>'
    );
...
function translatePage(lang) {
  jQuery(".current-language-flag").removeClass(
    getFlagClassFromLangCode(getCurrentLanguage())
  );
  jQuery(".widget").localizationTool("translate", lang);
  jQuery(".current-language-flag").addClass(
    getFlagClassFromLangCode(getCurrentLanguage())
  );
